...
object StaticPageUtil {
  ...
  def generatePublicWhosComingHtml(
    ...
    s"""
      ...
      |
      |       <tbody>
      |        ${thisWeekVolunteers.zipWithIndex.map {case (v, i) => s"<tr> <th scope=\"row\">${i+1}</th> <td>${v.firstname}</td> <td>${v.surname}</td> </tr>"}}
      |       </tbody>
      |    </table>
      |
      ...
      |
      |       <tbody>
      |        ${nextWeekVolunteers.zipWithIndex.map { case (v, i) => s"<tr> <th scope=\"row\">${i + 1}</th> <td>${v.firstname}</td> <td>${v.surname}</td> </tr>" }}
      |       </tbody>
      |    </table>
      |
